	...
	describe API do
		...
		describe 'POST api/v1/records' do
			context 'with valid data' do
				...
				before(:each) { browser.post 'api/v1/records', record_params }
		...
		describe 'GET /api/v1/color' do
			before(:each) { browser.get '/api/v1/color' }
			it 'returns a 200 status code' do
		    	expect(browser.last_response.status).to eq(200)
		    end
		...
		describe 'GET /api/v1/birthdate' do
			before(:each) { browser.get 'api/v1/birthdate' }
			it 'returns a 200 status code' do
		    	expect(browser.last_response.status).to eq(200)
		    end
		...
		describe 'GET /api/v1/surname' do
			before(:each) { browser.get 'api/v1/surname' }
			it 'returns a 200 status code' do
		    	expect(browser.last_response.status).to eq(200)
		    end
